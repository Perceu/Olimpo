...
class m_registro_entradas extends CI_Model {
    ...
    public function buscaMovimentosDia($dia = 0, $mes = 0, $ano = 0, $filtraConta = True)
    {
        ...
        if (($dia==0)||($mes==0)||($ano==0)){
            $sql = "SELECT * 
                    ,(select aluNome from alunos_carnes inner join alunos on alunos.aluId = alunos_carnes.aluId where reId  = registroentradas.reId) as nome
        ...
        $sql .= " ORDER by reData,turnos.turId,contas.conId,registrocategorias.rcId";
    ...
    public function entradas_por_categoria($mes = 0,$ano = 0, $dia = 0, $filtraConta = True){
        ...
        if ($mes==0 and $ano==0 and $dia==0){
            $sql = "SELECT rcid, rcNome, rcDescontaCaixa, sum(reValor) as valor,conNome, turNome FROM registroentradas 
                    inner join registrocategorias on registrocategorias.rcId = registroentradas.reCategoria
                    inner join contas on registroentradas.conId = contas.conId
                    inner join turnos on registroentradas.turId = turnos.turId
                    WHERE date_format(reData,'%Y%m') = date_format(now(),'%Y%m')";
    ...
    public function entradas_totais_por_categoria($mes = 0,$ano = 0, $dia = 0, $filtraConta = True){
        $this->load->database();
        if ($mes==0 and $ano==0 and $dia==0){
            $sql = "SELECT rcid, rcNome, rcDescontaCaixa, sum(reValor) as valor FROM registroentradas 
                    inner join registrocategorias on registrocategorias.rcId = registroentradas.reCategoria                   
                    WHERE date_format(reData,'%Y%m') = date_format(now(),'%Y%m')";
                    if ($filtraConta){
                        if ($this->session->userdata['Conta'] != 0 and $this->session->userdata['Perfil'] != 0){
